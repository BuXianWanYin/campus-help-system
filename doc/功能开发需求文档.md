...
**验证码使用Redis存储，不使用数据库表。**

**原因：**
- 验证码是临时数据，有有效期（5分钟），使用后即失效
- Redis支持自动过期，无需手动清理过期数据
- Redis读写性能更好，适合高频验证码操作
- 减少数据库压力，验证码操作不占用数据库资源

**Redis存储格式：**
- Key格式：`verification:code:{type}:{target}`
  - `type`：验证码类型（REGISTER-注册，LOGIN-登录，RESET_PASSWORD-重置密码）
  - `target`：目标邮箱
- Value：验证码字符串（6位数字）
- 过期时间：5分钟（自动过期）

**多验证码支持：**
- **同一邮箱可以同时存在多个不同类型的验证码**
- 不同验证码类型使用不同的Redis key，互不干扰
- 例如：用户`user@example.com`可以同时拥有：
  - 注册验证码：`verification:code:REGISTER:user@example.com` → `123456`
  - 登录验证码：`verification:code:LOGIN:user@example.com` → `654321`
  - 重置密码验证码：`verification:code:RESET_PASSWORD:user@example.com` → `789012`
- **同一类型的验证码会覆盖**：如果用户重复发送同一类型的验证码，新的验证码会覆盖旧的验证码

**示例：**
- 注册验证码：`verification:code:REGISTER:user@example.com` → `123456`（5分钟后自动过期）
- 登录验证码：`verification:code:LOGIN:user@example.com` → `654321`（5分钟后自动过期）
- 重置密码验证码：`verification:code:RESET_PASSWORD:user@example.com` → `789012`（5分钟后自动过期）

**实现逻辑：**
1. **发送验证码**：
   - 生成验证码
   - 存储到Redis：`SET verification:code:{type}:{target} {code} EX 300`（300秒=5分钟）
   - 如果已存在同类型验证码，会自动覆盖（Redis的SET操作会覆盖旧值）
2. **验证验证码**：
   - 从Redis获取：`GET verification:code:{type}:{target}`
   - 比较验证码是否匹配
   - 验证成功后删除：`DEL verification:code:{type}:{target}`（防止重复使用）
3. **验证码过期**：
   - Redis自动过期，无需手动清理
   - 过期后GET返回null，验证失败
...
**限流策略：**
- **同一邮箱同一类型**：60秒内只能发送1次
- **同一IP**：60秒内最多发送3次（所有类型合计）
- **同一邮箱所有类型**：60秒内最多发送5次

**Redis限流Key格式：**
- 邮箱类型限流：`verification:limit:{type}:{email}`（例如：`verification:limit:REGISTER:user@example.com`）
- IP限流：`verification:limit:ip:{ip}`（例如：`verification:limit:ip:192.168.1.1`）
- 邮箱总限流：`verification:limit:total:{email}`（例如：`verification:limit:total:user@example.com`）
...
public class VerificationCodeServiceImpl {
    ...
    public String sendVerificationCode(String type, String email, String clientIp) {
        ...
        String emailTypeCount = redisTemplate.opsForValue().get(emailTypeKey);
        if (emailTypeCount != null) {
            long remainingSeconds = redisTemplate.getExpire(emailTypeKey);
            throw new BusinessException("验证码发送过于频繁，请" + remainingSeconds + "秒后重试");
        }
        ...
        if (ipCountInt >= ipLimit) {
            long remainingSeconds = redisTemplate.getExpire(ipKey);
            throw new BusinessException("IP请求过于频繁，请" + remainingSeconds + "秒后重试");
        }
        
        // 3. 检查同一邮箱所有类型限流
        String emailTotalKey = "verification:limit:total:" + email;
        String emailTotalCount = redisTemplate.opsForValue().get(emailTotalKey);
        int emailTotalCountInt = emailTotalCount == null ? 0 : Integer.parseInt(emailTotalCount);
        if (emailTotalCountInt >= emailTotalLimit) {
            long remainingSeconds = redisTemplate.getExpire(emailTotalKey);
            throw new BusinessException("该邮箱验证码发送过于频繁，请" + remainingSeconds + "秒后重试");
        }
        
        // 4. 生成并发送验证码
        String code = generateCode();
        String codeKey = "verification:code:" + type + ":" + email;
        redisTemplate.opsForValue().set(codeKey, code, CODE_EXPIRE_MINUTES * 60, TimeUnit.SECONDS);
        
        // 5. 设置限流标记
        // 同一邮箱同一类型：60秒内只能发送1次
        redisTemplate.opsForValue().set(emailTypeKey, "1", limitWindowSeconds, TimeUnit.SECONDS);
        
        // 同一IP：增加计数
        redisTemplate.opsForValue().increment(ipKey);
        redisTemplate.expire(ipKey, limitWindowSeconds, TimeUnit.SECONDS);
        
        // 同一邮箱所有类型：增加计数
        redisTemplate.opsForValue().increment(emailTotalKey);
        redisTemplate.expire(emailTotalKey, limitWindowSeconds, TimeUnit.SECONDS);
...
<template>
  <el-button 
    :disabled="codeDisabled || countdown > 0"
    :loading="sending"
...
const handleSendCode = async () => {
  ...
  if (codeDisabled.value || countdown.value > 0 || sending.value) {
    return
  }
  ...
  try {
    await authApi.sendCode(props.type, props.email)
...
// 使用防抖包装发送验证码函数（1秒内只能触发一次）
export const debounceSendCode = debounce(async (sendCodeFn) => {
  await sendCodeFn()
}, 1000)
```
...
axios.interceptors.response.use(
  ...
  error => {
    ...
    if (error.response?.status === 429 || 
        ...
        error.response?.data?.message?.includes('过于频繁')) {
...
**配置说明：**
验证码相关配置在`application-dev.yml`中：
```yaml
verification:
  code:
    expire-minutes: 5  # 验证码过期时间（分钟）
    length: 6  # 验证码长度
    redis-key-prefix: "verification:code:"  # Redis key前缀
    limit:
      window-seconds: 60  # 限流时间窗口（秒）
      email-type-limit: 1  # 同一邮箱同一类型限制次数
      ip-limit: 3  # 同一IP限制次数
      email-total-limit: 5  # 同一邮箱所有类型限制次数
```
