...
mcp_mysql-campus_help_connect_db({
  host: "localhost",
  user: "root",
  password: "123456",  // 或 "123456"，根据实际配置
...
### 2.1 表命名规范
- 表名使用小写字母和下划线，如：`user`、`lost_found`、`order_info`
- 表名必须具有业务含义，清晰表达表的用途
...
// 分页查询
mcp_mysql-campus_help_query({
  sql: "SELECT * FROM user WHERE delete_flag = 0 ORDER BY create_time DESC LIMIT ? OFFSET ?",
  params: [10, 0]
})
```
...
// 1. 根据ID查询
mcp_mysql-campus_help_query({
  sql: "SELECT * FROM user WHERE id = ? AND delete_flag = 0",
  params: [userId]
})

// 2. 根据邮箱查询
mcp_mysql-campus_help_query({
  sql: "SELECT * FROM user WHERE email = ? AND delete_flag = 0",
  params: [email]
})

// 3. 条件查询
mcp_mysql-campus_help_query({
  sql: "SELECT * FROM user WHERE status = ? AND is_verified = ? AND delete_flag = 0",
  params: [1, 1]
})

// 4. 模糊查询
mcp_mysql-campus_help_query({
  sql: "SELECT * FROM user WHERE nickname LIKE ? AND delete_flag = 0",
  params: [`%${keyword}%`]
})

// 5. 统计查询
mcp_mysql-campus_help_query({
  sql: "SELECT COUNT(*) as total FROM user WHERE delete_flag = 0"
})
```
...
mcp_mysql-campus_help_execute({
  sql: "INSERT INTO user (password, email, nickname, create_time, update_time, delete_flag) VALUES (?, ?, ?, NOW(), NOW(), 0)",
  params: ["encrypted_password", "test@example.com", "测试用户"]
})
```
...
mcp_mysql-campus_help_execute({
  sql: "INSERT INTO user (password, email, nickname, create_time, update_time, delete_flag) VALUES (?, ?, ?, NOW(), NOW(), 0)",
  params: ["pwd1", "user1@example.com", "用户1"]
})

mcp_mysql-campus_help_execute({
  sql: "INSERT INTO user (password, email, nickname, create_time, update_time, delete_flag) VALUES (?, ?, ?, NOW(), NOW(), 0)",
  params: ["pwd2", "user2@example.com", "用户2"]
})
...
```javascript
mcp_mysql-campus_help_execute({
  sql: "UPDATE user SET nickname = ?, update_time = NOW() WHERE id = ? AND delete_flag = 0",
  params: ["新昵称", userId]
})
```
...
// 2. 更新多个字段
mcp_mysql-campus_help_execute({
  sql: "UPDATE user SET nickname = ?, email = ?, update_time = NOW() WHERE id = ? AND delete_flag = 0",
  params: ["新昵称", "newemail@example.com", userId]
})

// 3. 更新状态
mcp_mysql-campus_help_execute({
  sql: "UPDATE user SET status = ?, update_time = NOW() WHERE id = ? AND delete_flag = 0",
  params: [0, userId]
})

// 4. 条件更新
mcp_mysql-campus_help_execute({
  sql: "UPDATE user SET credit_score = credit_score + ? WHERE id = ? AND delete_flag = 0",
  params: [10, userId]
})
```
...
```javascript
mcp_mysql-campus_help_execute({
  sql: "UPDATE user SET delete_flag = 1, update_time = NOW() WHERE id = ? AND delete_flag = 0",
  params: [userId]
})
```
...
try {
  // 执行操作1
  mcp_mysql-campus_help_execute({
    sql: "INSERT INTO order (user_id, total_amount, create_time, update_time, delete_flag) VALUES (?, ?, NOW(), NOW(), 0)",
...
```javascript
mcp_mysql-campus_help_query({
  sql: "SELECT * FROM user WHERE delete_flag = 0 LIMIT 10"
})
```
...
// 逻辑删除所有数据
mcp_mysql-campus_help_execute({
  sql: "UPDATE user SET delete_flag = 1, update_time = NOW()"
})
...
# 备份指定表
mysqldump -u root -p campus_help user > user_backup.sql
```
...
# 恢复指定表
mysql -u root -p campus_help < user_backup.sql
```
