...
1. **MainLayout.vue**（主布局）：用于需要顶部导航栏的页面（首页、列表页、详情页等）
   - 包含：顶部导航栏、主内容区、底部导航栏（移动端）、消息通知、返回顶部按钮
   - 适用页面：首页、失物招领、闲置交易、跑腿服务等业务页面

2. **BasicLayout.vue**（基础布局）：用于简单的管理页面
   - 包含：顶部导航栏、主内容区
   - 适用页面：个人中心、设置等页面

3. **无布局**：用于认证页面（登录、注册、忘记密码）
   - 这些页面使用独立的左右分栏布局

#### 2.0.3 MainLayout 布局结构
...
#### 2.0.9 HTML布局转Vue组件指南

**转换步骤：**

1. **分析HTML结构**
   - 识别布局容器（header、main、nav等）
   - 识别可复用组件（卡片、列表项等）
   - 识别交互功能（搜索、下拉菜单等）

2. **创建布局组件**
   - 在 `src/layouts/` 目录创建布局组件
   - 将HTML的布局结构转换为Vue模板
   - 使用Element Plus组件替换HTML原生元素

3. **样式转换**
   - Tailwind CSS类名 → 自定义CSS类
   - 保持相同的视觉效果
   - 使用项目统一的颜色和间距规范

4. **功能实现**
   - 使用Vue的响应式数据（`ref`、`reactive`）
   - 使用Vue Router进行路由跳转
   - 使用Pinia进行状态管理
...
<header class="bg-white shadow-sm sticky top-0 z-50">
  <div class="container mx-auto px-4 py-3 flex items-center justify-between">
    <h1 class="text-xl font-bold text-primary">校园帮</h1>
  </div>
</header>
```

```vue
<!-- Vue (Element Plus + 自定义CSS) -->
<template>
  <header class="main-header">
    <div class="header-container">
...
| `text-primary` | `color: #769fcd` | 主色文字 |
| `hover:bg-gray-100` | `:hover { background-color: #f7fbfc }` | 悬停效果 |
...
- **整体布局架构规范**（MainLayout、BasicLayout）
- 项目结构设计
- 组件开发规范
- API接口调用规范
- 状态管理规范（Pinia）
