...
- **内容审核**：所有用户发布的内容（失物招领、闲置商品、跑腿任务等）必须使用智能审核机制（敏感词过滤）

#### 5.3.1 内容发布智能审核机制（强制要求）

##### 5.3.1.1 适用范围
以下内容发布功能**必须**使用智能审核机制：
- ✅ 失物招领发布（`LostFoundService.publishLostFound`）
- ✅ 失物招领编辑（`LostFoundService.updateLostFound`）
- ✅ 闲置商品发布（`GoodsService.publishGoods`）
- ✅ 闲置商品编辑（`GoodsService.updateGoods`）
- ✅ 跑腿任务发布（`ErrandTaskService.publishTask`）
- ✅ 跑腿任务编辑（`ErrandTaskService.updateTask`）

##### 5.3.1.2 实现要求
1. **必须使用`SensitiveWordService.check()`方法**进行敏感词检测
2. **检测范围**：标题和描述内容（拼接后进行检测）
3. **检测流程**：
   ```
   敏感词检测（AC自动机算法）
       ↓
   检测结果判断：
       ...
       │   - 设置audit_status = "PENDING"
       │   - 填充audit_trigger_reason（触发原因）
       │
       └─ 无敏感词 → 继续其他检测
           - 发布频率检测
           - 新用户检测（注册7天内）
...
public class GoodsServiceImpl implements GoodsService {
    ...
    public Long publishGoods(GoodsDTO dto, Long userId) {
        ...
        if (!checkResult.isPass() && !checkResult.isNeedManualReview()) {
            ...
            goods.setAuditReason(checkResult.getMessage()); // 自动填充拒绝原因
        ...
        if (checkResult.isPass() && frequencyOk && !newUser) {
            ...
            goods.setAuditTriggerReason("包含敏感词；"); // 记录触发原因
        }
...
##### 5.3.1.4 注意事项
- **必须注入`SensitiveWordService`**：所有内容发布的Service必须注入敏感词服务
- **必须保存拒绝记录**：即使被自动拒绝，也要保存记录到数据库，方便用户在"我的发布"页面查看
- **必须填充拒绝原因**：自动拒绝时必须自动填充`audit_reason`字段，说明拒绝原因
- **必须记录触发原因**：转人工审核时必须填充`audit_trigger_reason`字段，说明触发审核的原因
- **编辑时重新检测**：编辑已发布内容时，必须重新进行敏感词检测
