      ...
      <div v-else class="question-items">
        <div v-for="item in questionList" :key="item.id" class="question-item">
          ...
          <div class="question-actions">
            ...
            <el-button 
              v-if="item.status !== 'SOLVED' && item.status !== 'CANCELLED' && item.status !== 'REJECTED'"
              type="warning" 
              text 
              size="small" 
...
import { Plus, Collection, Money, ChatDotRound, View, Clock } from '@element-plus/icons-vue'
...
const handleCancel = async (item) => {
  try {
    ...
    const response = await questionApi.cancel(item.id)
    if (response.code === 200) {
